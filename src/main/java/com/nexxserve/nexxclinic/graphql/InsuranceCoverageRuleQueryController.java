...
@Controller
public class InsuranceCoverageRuleQueryController {

    private final InsuranceCoverageRuleService ruleService;

    public InsuranceCoverageRuleQueryController(InsuranceCoverageRuleService ruleService) {
        this.ruleService = ruleService;
    }

    @HasRole({RoleName.ADMIN, RoleName.CLINIC_ADMIN, RoleName.RECEPTION, RoleName.FINANCE})
    @QueryMapping
    public ApiResponse insuranceCoverageRule(
    ...
    @HasRole({RoleName.ADMIN, RoleName.CLINIC_ADMIN, RoleName.RECEPTION, RoleName.FINANCE})
    @QueryMapping
    public ApiResponse insuranceCoverageRules(
        @Argument SearchInsuranceCoverageRulesInput input,
        @ContextValue(name = "authUser", required = false) AuthenticatedUser authUser
    ) {
